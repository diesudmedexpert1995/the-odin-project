...
class GameController {
  ...
  constructor() {
    ...
    MainView.drawPreGameShips([2, 3, 3, 4, 5]);
    this.addPreGameDraggableEvents();
    MainView.drawPreGameRandomButton();
      ...
      draggable.addEventListener("dragend", () => {
        ...
        if (currentDropLength === currentDropCoordinates.length) {
          ...
          if (checkShips.length === 0) {
            MainView.drawPreGameStartButton();
  ...
  updateBoards() {
    for (const [player, el] of this.#playerDOM.entries()) {
      MainView.drawBoard(el);
      MainView.drawShips(
        el,
        player.gameboard.ships.reduce((a, b) => a.concat(b.coordinates), []),
      );
      MainView.drawShots(el, player.gameboard.shots);
