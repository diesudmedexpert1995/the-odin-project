import Gameboard from "../src/modules/models/Gameboard.js";
...
describe("Gameboard", () => {
  ...
  test("placeShip() throws Error if length of the ship and coordinates are not equal", () => {
    ...
    expect(() => gameboard.placeShip(ship, [])).toThrow(
      "The ship`s length should be same as given coordinates length",
    );
  });
  ...
  test("placeShip() throws Error if same ship is inserted more than once", () => {
    ...
    }).toThrow("Same ship exists in this coordinates on the gameboard");
  });
